...
namespace Oculus.Interaction.Samples
{
    public class Slingshot : MonoBehaviour, ITransformer
    {
        ...
        private void OnTriggerEnter(Collider other)
        {
            ...
            {
                lePelletSnapped(pellet);
            }
        }


        private void lePelletSnapped(SlingshotPellet pellet)
        {
            GrabInteractor GrabInteractor = pellet.Grabber;
            if (GrabInteractor == null
                || GrabInteractor.State != InteractorState.Select)
            {
                return;
            }
            foreach (var interactable in _GrabInteractables)
            {
                if (GrabInteractor.CanInteractWith(interactable))
                {
                    GrabInteractor.ForceRelease();
                    GrabInteractor.ForceSelect(interactable, true);
